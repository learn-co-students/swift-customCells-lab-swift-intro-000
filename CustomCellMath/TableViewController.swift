...
class TableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let arrayTitle = numbers[indexPath.row]
        
        cell.firstNumberLabel?.text = String(arrayTitle[0])
        cell.secondNumberLabel?.text = String(arrayTitle[1])
        cell.thirdNumberLabel?.text = String(arrayTitle[2])
        cell.fourthNumberLabel?.text = String(arrayTitle[3])
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier != "ShowMath" { return }
