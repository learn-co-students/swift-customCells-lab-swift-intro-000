...
class TableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.firstNumberLabel.text? = String(numberList[0])
        cell.secondNumberLabel.text? = String(numberList[1])
        cell.thirdNumberLabel.text? = String(numberList[2])
        cell.fourthNumberLabel.text? = String(numberList[3])
        
        return cell
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier != "displayMathDetails" { return }
