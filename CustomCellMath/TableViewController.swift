...
class TableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let numberSet = numbers[indexPath.row]
        cell.firstNumberLabel.text = String(numberSet[0])
        cell.secondNumberLabel.text = String(numberSet[1])
        cell.thirdNumberLabel.text = String(numberSet[2])
        cell.fourthNumberLabel.text = String(numberSet[3])
        
        return cell
    }

    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier != "DisplayMathView" {return}
