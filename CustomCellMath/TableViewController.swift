...
class TableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath)-> UITableViewCell {
        ...
        let currentCell = numbers[indexPath.row]
        cell.firstNumberLabel?.text = String(currentCell[0])
        cell.secondNumberLabel?.text = String(currentCell[1])
        cell.thirdNumberLabel?.text = String(currentCell[2])
        cell.fourthNumberLabel?.text = String(currentCell[3])
        
        return cell
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier != "MathViewSegue" {return}
