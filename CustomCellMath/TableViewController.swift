...
class TableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let numberArr = numbers[(indexPath as NSIndexPath).row]
        cell.firstNumberLabel?.text = String(describing: numberArr[0])
        cell.secondNumberLabel?.text = String(describing: numberArr[1])
        cell.thirdNumberLabel?.text = String(describing: numberArr[2])
        cell.fourthNumberLabel?.text = String(describing: numberArr[3])
        return cell
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier != "DisplayMathViewController" { return }
